...
export const CirclePacking: React.FC<CirclePackagingProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const fontSize = (d: any) =>
      Math.min((2 * d.r) / (d.data.name.length * 0.5), d.r * 0.7) + "px";
    const fontAndStrokeColor = (d: any) => `hsl(${d.data.color}, 100%, 70%)`;
    const backgroundColor = (d: any) => `hsla(${d.data.color}, 100%, 15%, 0.5)`;
    ...
    // Add a title.
    node.append("title").text((d) => `${d.data.name} (${format(Number(d.value))})`);
