...
function bilink(
  root: d3.HierarchyNode<EdgeBundling>,
): ExtendedHierarchyNode<EdgeBundling> {
  const map: Map<string, ExtendedHierarchyNode<EdgeBundling>> = new Map(
    root.leaves().map((d) => [d.data.name, d]),
  )
  for (const d of root.leaves()) {
    ;(d as ExtendedHierarchyNode<EdgeBundling>).outgoing =
      d.data.connections.map((i) => [d, map.get(i.name)!])
  }
  return root
}
...
const HierarchicalEdgeBundling: React.FC<Props> = ({ data }) => {
  ...
  function overed(
    _event: unknown,
    d: ExtendedHierarchyPointNode<EdgeBundling>,
  ) {
  ...
  function outed(
    ...
    d3.selectAll(
      d.outgoing?.map((d) => d.pathElement!).filter(Boolean) || [],
    ).attr('stroke', colornone) // Restablece el color de los enlaces salientes a gris claro
  }
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      const svg = d3
        ...
        .attr(
          'style',
          'width: 100%; height: auto; font-size: 14px; max-height: 100vh;',
        )
      ...
      svg
        ...
        .text(
          (d) =>
            `${d.data.name}${
              d.data.difference ? ` (${d.data.difference})` : ''
            }`,
        )
        .attr('fill', colorTag)
        .each(function (d) {
